...
class KeyStore {
  ...
  constructor(cluster) {
    ...
    // - key updates also wait behind sync to joining members
  ...
  createStore(name) {
    ...
    store.actions.set = (key, value, opts) => { // no opts at this time
      let type = typeof key;
    ...
    store.actions.get = (key) => {
      return store.data[key] ? store.data[key].val : undefined;
    };
