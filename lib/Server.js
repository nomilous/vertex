...
class Server {
  ...
  _onConnection(socket, direction = 'from') {
    let address = socket.remoteAddress();
    this.log.debug('connected %s %s:%s', direction, address.address, address.port);
    ...
    let onClose = (code, message) => {
      ...
      socket.closed = true; // TODO: move into vertex-transport (or find connected flag)

      if (!socket.member) return;
      socket.member._quarantine();
    };
